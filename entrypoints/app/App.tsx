...
export default function App() {
  ...
  async function openArchive(file: File) {
    if (!file.name.toLowerCase().endsWith('.zip')) {
      setTypeHint("That's not a .zip — drop a ZIP file.");
      return;
    }
    setTypeHint('');
    try {
    ...
    <main className="min-h-screen bg-[radial-gradient(circle_at_top_left,#e5f5e7,transparent_35%),#f8faf6] px-5 py-10">
      <div className="mx-auto max-w-4xl">
        ...
        <header className="mb-10 flex items-start justify-between gap-6">
          ...
          <div className="flex-shrink-0 rounded-full border border-emerald-200 bg-white px-3 py-1.5 text-xs font-semibold text-emerald-800">
            100% offline
          </div>
        </header>
        ...
        {status === 'idle' && (
          <>
            <Dropzone ref={dropzoneRef} onFile={openArchive} />
            {typeHint && (
              <p className="mt-3 text-center text-sm text-amber-700" aria-hidden="true">
                {typeHint}
              </p>
            )}
          </>
